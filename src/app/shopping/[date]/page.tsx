...
export default function ShoppingPage() {
    ...
    const toggleItemCompleted = (sku: string) => {
        setShoppingList(prev => prev.map(item =>
            item.sku === sku
                ? { ...item, completed: !item.completed, purchased: !item.completed ? item.totalNeeded : 0 }
                : item
        ))
    }
                        ...
                        <CardContent className="p-4">
                            <div className="flex items-center gap-4">
                                ...
                                <div className="flex-1 min-w-0">
                                    <h3 className={`font-medium text-lg leading-tight mb-1 ${item.completed ? 'text-green-800 line-through' : 'text-gray-900'
                                        }`}>
                                        {item.name}
                                    </h3>
                ...
                <Card>
                    <CardContent className="p-8 text-center">
                        <p className="text-gray-600 mb-4">
                            {searchTerm ? 'No products found matching your search' : 'No products found for this date'}
                        </p>
                        {searchTerm && (
                            <Button
